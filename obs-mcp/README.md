...
```shell
$ helm install prometheus-community/prometheus --name-template local # sets up Prometheus (and exporters) on your local single-node k8s cluster
$ export POD_NAME=$(kubectl get pods --namespace default -l "app.kubernetes.io/name=alertmanager,app.kubernetes.io/instance=local" -o jsonpath="{.items[0].metadata.name}")
